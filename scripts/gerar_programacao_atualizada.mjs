/**
 * Script para gerar um novo arquivo XLSX com base nas informações extraídas do PDF
...
// Adicionar cabeçalho
wsProgramacao.addRow(['', ...wsProgramacao.columns.slice(1).map(col => col.header)]);
...
// Adicionar cabeçalho
wsPalestrantes.addRow(['', ...wsPalestrantes.columns.slice(1).map(col => col.header)]);
...
// Adicionar cabeçalho
wsStands.addRow(['', ...wsStands.columns.slice(1).map(col => col.header)]);
...
// Salvar workbook
wb.xlsx.writeFile('./src/data/programacao.xlsx').then(() => {
  ...
  // Agora gerar o JSON correspondente
  const str = (val) => {
    if (val == null) return '';
  ...
  // ── Stands ────────────────────────────────────────────────────────────────
  const stands = [];
  const wsStandsSheet = wb.getWorksheet('Stands');
  wsStandsSheet?.eachRow((row, rn) => {
    if (rn === 1) return;
  ...
  writeFileSync('./src/data/programacao-gerado.json', JSON.stringify(out, null, 2), 'utf-8');
